...
# Only these statuses are valid for the UI
...
def s3_client():
    if not S3_BUCKET:
        st.error("S3_BUCKET is not configured in Streamlit secrets."); st.stop()
    if not (AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY):
        st.error("AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY not set in Streamlit secrets."); st.stop()
    return boto3.client(
        "s3",
...
def _s3_head_etag(bucket: str, key: str) -> str:
    ...
    try:
        ...
        return "no-etag"  # lets the app still render with history
...
def load_vf_latest_from_s3(cache_bust: str) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(f"Could not read latest snapshot from s3://{S3_BUCKET}/{key} — showing history only.\n\n{e}")
        return pd.DataFrame()
    ...
    for k in keys:
        try:
            ...
            if "scraped_at_utc" not in df.columns:
                # derive from filename vf_snapshot_YYYYMMDDTHHMMSSZ.csv
...
def add_time_bins(df: pd.DataFrame) -> pd.DataFrame:
    if "scraped_at_utc" not in df.columns: return df
...
def latest_timestamp(df: pd.DataFrame) -> Optional[datetime]:
    if "scraped_at_utc" not in df.columns or df.empty: return None
...
def group_counts(df: pd.DataFrame, status: str, freq: str, ship_types: List[str]) -> pd.DataFrame:
    ...
    dfx = df.copy()
    if status: dfx = dfx[dfx["status"] == status]
    if ship_types: dfx = dfx[dfx["ship_type"].isin(ship_types)]
    if dfx.empty: return dfx
    dfx = dfx.dropna(subset=["scraped_at_utc"]).set_index("scraped_at_utc")
    grouped = (dfx.groupby("ship_type").resample(freq)["mmsi"].nunique()
...
df_all = unify_schema(df_all)

# 🔒 Hard filter to valid statuses only (drops 'unknown' like demo rows)
df_all = df_all[df_all["status"].isin(KNOWN_STATUSES)]

# 🧹 Kill any leftover demo row by name, just in case history still has one
if "name" in df_all.columns:
    df_all = df_all[df_all["name"].str.lower().ne("demo vessel")]

# Deduplicate and time-bucket
df_all = df_all.drop_duplicates(subset=["mmsi","scraped_at_utc"], keep="last")
...
with st.expander("🔧 Debug – source & counts"):
    ...
    if not df_all.empty:
        st.write("Statuses (after filter):", df_all["status"].value_counts(dropna=False))

# If latest is empty, tell the user we’re showing history only
if vf_latest.empty:
    st.warning("Latest VF snapshot is empty or missing — showing historical files only. "
               "Check the GitHub Action preflight counts and ensure it uploaded a non-empty CSV.")
